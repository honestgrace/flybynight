...
#define INIT_ALLOCATOR_LHTSUBCLASS(CLASS, N)                    \
	...
	{
		//
		// delete allocations that succeeded to avoid memory leaks upon failure
		//
		LKRHashTableUninit();
	}
...
namespace LKRhash {
			...
			int&         riNode)    // index within node
	{
		...
		__analysis_assume(0 <= iNode2 && iNode2 < NODES_PER_CLUMP);
		__analysis_assume(0 <= riNode && riNode < NODES_PER_CLUMP);
...
#include <stdlib.h>
	...
	{
		// 
		// make sure math below happens on 64 bits to prevent
		// theoretical arithmetic overflow
		//
		if (m_cActiveBuckets >= MAX_DIRSIZE * (ULONGLONG)m_dwSegSize - 1)
			return LK_ALLOC_FAIL;  // table is not allowed to grow any more

		WriteLock();

		// double segment directory size if necessary
		if (m_cActiveBuckets >= (ULONGLONG)m_cDirSegs * m_dwSegSize)
		{
			IRTLASSERT(m_cDirSegs < MAX_DIRSIZE);

			if ((m_cDirSegs * 2) < m_cDirSegs ||
				(m_cDirSegs * 2) < 2)
			{
				WriteUnlock();
			...
			DWORD cDirSegsNew = (m_cDirSegs == 0) ? MIN_DIRSIZE : m_cDirSegs * 2;
		...
		while (pncOldList != NULL)
		{
			...
			FOR_EACH_NODE(i)
			{
				...
				if (dwBkt == dwNewBkt)
				{
					...
					if (iNewSlot == NODE_END || iNewSlot == NODES_PER_CLUMP)
					{
				...
				else
				{
					...
					if (iOldSlot == NODE_END || iOldSlot == NODES_PER_CLUMP)
					{
						// the calling routine has passed in a FreeList adequate
						// for all needs
						IRTLASSERT(pncFreeList != NULL);
		...
		while (pncOldList != NULL)
		{
			...
			FOR_EACH_NODE(i)
			{
				...
				found_slot:
					...
					// NOTE: Adding this to help OACR's analysis.  The situation
					// that the OACR originally flagged (Windows OS #1701248) can't
					// arise with the values it gives in the bug. 
					__analysis_assume(0 <= iNewSlot && iNewSlot < NODES_PER_CLUMP);
