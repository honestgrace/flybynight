...
DWORD WINAPI TestIOCPWinSockServer_ServerWorkerThread(LPVOID CompletionPortID)
{
...
int TestIOCPWinSockServer(int argc, char **argv)
{
	...
	for (i = 0; i < (int)SystemInfo.dwNumberOfProcessors * 2; i++)
	{
		...
		if ((ThreadHandle = CreateThread(NULL, 0, TestIOCPWinSockServer_ServerWorkerThread, CompletionPort, 0, &ThreadID)) == NULL)
		{
