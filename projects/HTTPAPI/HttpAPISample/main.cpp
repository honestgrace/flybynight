...
DWORD DoReceiveRequests(IN HANDLE hReqQueue, bool use_HTTPAPI_VERSION_2);
...
)
{
    ...
    HTTP_URL_CONTEXT urlContext = NULL;

	int UrlAdded = 0;
    bool use_HTTPAPI_VERSION_2 = true;

    HTTPAPI_VERSION HttpApiVersion = {};

    if (use_HTTPAPI_VERSION_2)
    {
	    HttpApiVersion = HTTPAPI_VERSION_2;
    }
    else
    {
        HttpApiVersion = HTTPAPI_VERSION_1;
    }
    ...
    else
    {
        ...
        StateInfo.State = HttpEnabledStateInactive;
        ...
        ConfigGroupState.State = HttpEnabledStateInactive;
    ...
    // The command line arguements represent URIs that to listen on.
	...
	for (int i = 1; i < argc; i++)
	{
        ...
        else
        {
            ...
            HTTP_ENABLED_STATE HttpReason = HTTP_ENABLED_STATE::HttpEnabledStateInactive;
            ...
            HttpReason = HTTP_ENABLED_STATE::HttpEnabledStateActive;
            retCode = HttpSetRequestQueueProperty(hReqQueue,
                HttpServerStateProperty,
                reinterpret_cast <VOID*> (&HttpReason),
                sizeof(HTTP_ENABLED_STATE),
		...
		DoReceiveRequests(hReqQueue, use_HTTPAPI_VERSION_2);
	}
