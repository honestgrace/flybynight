...
function Prepare-TestEnv() {
    ...
    { 
        ...
        if ($iisinstalled)
        {
            ...
            New-Website -Name $testSiteName -Port 555 -PhysicalPath c:\inetpub\wwwroot
	    ...
	    $mypwd2 = ConvertTo-SecureString -String $mypwd2Text -Force -AsPlainText
            ...
            if ( -not $iisadministrationAvailable ) {
                if (1 -eq (get-webbinding -name $testSiteName).count) {
                    # Set SSL with Central Certificate, sslFlag 2: central certificate only
                    New-WebBinding -Name $testSiteName -IP "*" -Port 443 -Protocol https -SSLFlags 2 -HostHeader $dockerHost
                    New-Item -Path "IIS:\SslBindings\!443!$dockerHost" -sslFlags 2

                    # Set SSL with basic setting
                    #New-WebBinding -Name $testSiteName -IP "*" -Port 443 -Protocol https
                    #get-item $certificatePath | new-item -path IIS:\SslBindings\0.0.0.0!443
                }
            }
            else {
                if (1 -eq (Get-IisSiteBinding -Name $testSiteName).count) {
                    # Set SSL with Central Certificate, sslFlag 2: central certificate only
                    New-IISSiteBinding -Name $testSiteName -Protocol https -SslFlag 2 -BindingInformation "*:443:$dockerHost"

                    # Set SSL with basic setting
                    #New-IISSiteBinding -Name $testSiteName -Protocol https -CertificateThumbPrint $certificate.Thumbprint -BindingInformation "*:443:" -CertStoreLocation "My"
                }
            }
        }
    } 
...
function Basic-HttpTest() {
    ...
    $item = @{
        "url"="http://localhost/now.asp";   
        "expected"="Time:";
    }
    Invoke-TestIIS $item "now.asp"
