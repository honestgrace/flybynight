...
function Prepare-TestEnv() {
    ...
    { 
        ...
        if ($iisinstalled)
        {
            ...
            Export-PfxCertificate -FilePath "$env:systemdrive\certificates\localhost.pfx" -Cert $certificatePath -Password $mypwd
            Import-PfxCertificate -FilePath "$env:systemdrive\certificates\localhost.pfx" -CertStoreLocation "Cert:\LocalMachine\Root" -Password $mypwd
	    ...
	    $mypwd2 = ConvertTo-SecureString -String $mypwd2Text -Force -AsPlainText
            ...
            if ( -not $iisadministrationAvailable ) {
                if (1 -eq (get-webbinding -name "Default Web Site").count) {
                    ...
                    New-WebBinding -Name "Default Web Site" -IP "*" -Port 443 -Protocol https -SSLFlags 2 -HostHeader localhost
                    New-Item -Path "IIS:\SslBindings\!443!localhost" -sslFlags 2
                ...
                if (1 -eq (Get-IisSiteBinding -Name "Default Web Site").count) {
                    # Set SSL with Central Certificate, sslFlag 2: central certificate only
                    New-IISSiteBinding -Name "Default Web Site" -Protocol https -SslFlag 2 -BindingInformation "*:443:JHKIM-DEV"
