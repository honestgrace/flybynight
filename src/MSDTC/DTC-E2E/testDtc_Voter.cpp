...
int testDtc_Voter()
{
	...
	// Get Voter Factory
	//*******************
	ITransactionVoterFactory2 * pXATransactionVoterFactory2 = nullptr;
	HR(pXATransactionDispenser->QueryInterface(__uuidof(ITransactionVoterFactory2), (void **) &pXATransactionVoterFactory2));
	
	//*******************
	// Get Voter
	//*******************
	ITransactionVoterBallotAsync2 * pITransactionVoterBallotAsync2 = nullptr;
	TransactionVoterNotifyAsync2 transactionVoterNotifyAsync2;
	transactionVoterNotifyAsync2.AddRef();
    HR(pXATransactionVoterFactory2->Create(pXATransaction, &transactionVoterNotifyAsync2, &pITransactionVoterBallotAsync2));  
	transactionVoterNotifyAsync2.SaveContext(pITransactionVoterBallotAsync2);
